...
存在上述问题的 API 绝不止 wx.getLocation，遗憾的是，小程序并未开放清理缓存的接口。但可通过 __wx.openSetting__ 再次请求用户开启授权：
...
![](http://p20v1ngpr.bkt.clouddn.com/track-data.jpg)
...
rpx 在不同设备被小程序换算为 px 时能产生各种 bug（精度太低）：

```html
<view class="fliter-bar gm-clear" style="top: { { top } }rpx;"></view>
```

若 top = 0，0rpx 被换算为 0.5px 也是厉害：
...
```html
<view class="fliter-bar gm-clear" style="top: { { top ? (top + 'rpx') : 0 } };"></view>
```

当 fliter-bar 高度为 84rpx，对于 fliter-wrap：

```html
<view style="top: { { top + 84 } }rpx;"></view>
```

理论上紧贴的 fliter-bar 与 fliter-wrap 在部分设备上也不紧贴...
